...
- Mutexes, locks, and try-locks
  - Instead, use lock-free methods for sharing data between threads. There are three commonly used methods:
    ...
    - A lock-free single-producer single-consumer (spsc) ring buffer (message channel) - Some recommendations are [SPSCQueue](https://github.com/rigtorp/SPSCQueue) in C++, and [rtrb](https://github.com/mgeier/rtrb) or [ringbuf](https://crates.io/crates/ringbuf) in Rust.
